...
interface Props {
  /** true while the mouse is sliding between cells */
  isRunning:   boolean;
  /** true when last horizontal move was to the right */
  facingRight: boolean;
  /** cell size in px — the SVG fills this square */
  size:        number;
}

/**
 * A CSS-animated cartoon mouse drawn entirely in SVG.
 * Faces LEFT by default; flip via facingRight.
 *
 * Running cycle (triggered by isRunning):
 *   • Front legs and back legs alternate up/down
 *   • Body bounces slightly
 *   • Tail wags faster
 *
 * Idle cycle:
 *   • Gentle breathing (scaleY) and tail slow-sway
 */
export function MouseSprite({ isRunning, facingRight, size }: Props) {
  ...
  return (
    <svg
      ...
      aria-hidden="true"
      style={{ transform: facingRight ? 'scaleX(-1)' : undefined, overflow: 'visible' }}
    >
      {/* ── Tail (behind body) ─────────────────────────────── */}
      <path
        className="ms-tail"
      ...
      {/* ── Back-leg pair (painted behind body) ─────────────── */}
      ...
      {/* ── Front-leg pair (painted over body) ──────────────── */}

...
## 七、音效與彩帶系統（2026-05-12 新增）

### 7.1 音效設計（useSound.ts）

所有音效由 **Web Audio API** 即時合成，零外部檔案，零著作權問題。

| 音效 | 合成方式 | 特色 |
|------|---------|------|
| 移動 squeak | 三角波頻率掃描 640–800→350 Hz，130ms | 每次隨機微調音高，不單調 |
| 過關歡呼 | 白噪音群眾聲 + C5→E5→G5→C5→C6 旋律 + 8 個閃爍音 | 三層合成，持續約 1.4 秒 |

**重要原則：** `sound.playCheer()` 呼叫用 `try-catch` 包覆，音效失敗不影響彩帶顯示。

### 7.2 背景音樂（useBackgroundMusic.ts）

- 自創 8 秒旋律（C 大調五聲音階，120 BPM）
- Look-ahead 排程器（100ms 預讀，25ms tick），循環無縫接合
- 三角波主旋律 + 低八度正弦波伴奏
- 停止時 400ms 漸出

### 7.3 彩帶系統（Confetti.tsx / Confetti.css）

**設計：**
- 120 個長條碎片，3 種尺寸（長細 / 中等 / 寬短）
- 父子雙元素結構：外層處理飄落路徑（Y平移 + 左右搖擺），內層處理翻轉（scaleX 1→0.06，模擬 3D 旋轉）
- 落下速度 6.5–11 秒（夠慢，用戶能看到完整過程）
- 延遲最多 1.8 秒（碎片陸續出現，有層次感）
- CSS `animation-fill-mode: forwards` + wrap 在 6s 後 opacity→0

**關鍵 Bug 修復：**
- 原本用 `showConfetti` state + `useEffect([game.won])` 控制，但 React effect 排程延遲導致 state 永遠不變
- 修復：直接傳 `active={game.won}` 給 Confetti，去除中間狀態
- 驗證方法：`visible:120` — 贏得遊戲 300ms 後 JS 查詢確認 120 個碎片全在可見範圍
...
| `b188ae5` | docs: update memory.md |
| `5c16103` | feat: add sound effects and background music |
| `f370b4e` | feat: win celebration — confetti rain + cheering sound |
| `f86975c` | fix: confetti — longer strips, direct game.won binding |
| 最新 | docs: update skill.md and memory.md |
...
└── src/
    ...
    │   ├── useSound.ts              # squeak + cheer（Web Audio API）
    │   └── useBackgroundMusic.ts    # 8 秒循環旋律，look-ahead 排程
    └── data/
